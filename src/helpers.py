...
def pick_cells(sc_adata_ref, sn_adata, sc_adata_pseudos, min_cells_per_type=50, check_min_cells=True):
    """
...
def plot_controls(string_title="RMSE", string_column="RMSE", string_ylabel="RMSE", 
    ...
    sns.boxplot(x='DisplayTransform', y=string_column, data=df_controls, 
                        palette=control_colors, 
                        linewidth=7, 
                        fliersize=10)
    plt.title(f"Deconvolution Performance:\n{string_title}", fontsize=56, fontweight="bold")
    ...
    plt.xlabel("Reference Data Type",
...
def open_adipose_datasets_all(res_name="Real_ADP", base_dir=".."):
    ...
    matching = np.intersect1d(sn_adata.obs.cell_types.unique(), sc_adata.obs.cell_types.unique())
    matching

    nonmatching = np.setxor1d(sn_adata.obs.cell_types.unique(), sc_adata.obs.cell_types.unique())
    nonmatching
