...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--noise", type=bool, default=True, help="Add Gaussian noise to pseudobulks"
    )
    parser.add_argument(
        "--deseq_alpha",
        type=float,
    ...
    for cell_type in cell_types:
        ...
        ### 3) DEG-Filtered SN (intersect of 3 datsets) ####
        ###########################
        if args.res_name == "MSB":
            pass
        else:
        ...
        random_genes = np.random.choice(
            adata_sc_except_ct.var_names, size=len(all_diff_genes), replace=False
        )
